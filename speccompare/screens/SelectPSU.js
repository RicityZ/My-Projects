...
export default function SelectPSU({ route, navigation }) {
  ...
  useEffect(() => {
    fetch('http://10.0.2.2/speccompare-api/get-parts.php?part=psu')
      .then((res) => res.json())
      .then((json) => {
  ...
  const getBrandFromName = (name) => {
    ...
    if (lower.includes('antec')) return 'Antec';
    return 'Other';
  };

  const brands = ['Corsair', 'Thermaltake', 'ASUS', 'Cooler Master', 'Antec', 'Other'];
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>เลือก Power Supply</Text>
      ...
      <TextInput
        placeholder="ค้นหา Power Supply..."
