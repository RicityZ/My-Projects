...
export default function SelectSSD({ route, navigation }) {
  ...
  useEffect(() => {
    fetch('http://10.0.2.2/speccompare-api/get-parts.php?part=ssd')
      .then((res) => res.json())
      .then((json) => {
  ...
  const getBrandFromName = (name) => {
    ...
    if (lower.includes('wd') || lower.includes('western')) return 'Western Digital';
    if (lower.includes('crucial')) return 'Crucial';
    if (lower.includes('adata')) return 'ADATA';
    if (lower.includes('seagate')) return 'Seagate';
    if (lower.includes('pn')) return 'PNY';
    if (lower.includes('sandisk')) return 'SanDisk';
    return 'Other';
  };

  const brands = ['Samsung', 'Kingston', 'Western Digital', 'Crucial', 'ADATA', 'Seagate', 'PNY', 'SanDisk'];
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>เลือก Solid State Drive</Text>
