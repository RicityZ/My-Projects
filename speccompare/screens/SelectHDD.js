...
export default function SelectHDD({ route, navigation }) {
  ...
  useEffect(() => {
    fetch('http://10.0.2.2/speccompare-api/get-parts.php?part=hdd')
      .then((res) => res.json())
      .then((json) => {
  ...
  const getBrandFromName = (name) => {
    ...
    if (lower.includes('wd') || lower.includes('western')) return 'WD';
    if (lower.includes('toshiba')) return 'Toshiba';
    if (lower.includes('samsung')) return 'Samsung';
    return 'Other';
  };

  const brands = ['Seagate', 'WD', 'Toshiba', 'Samsung', 'Other'];
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>เลือก Hard Disk</Text>
