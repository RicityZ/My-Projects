...
import {
  View, Text, FlatList, StyleSheet, TouchableOpacity, TextInput, Image, ScrollView
} from 'react-native';
import CustomHeader from '../components/CustomHeader';
import { useUser } from '../context/UserContext';

export default function SelectMemory({ route, navigation }) {
  ...
  useEffect(() => {
    fetch('http://10.0.2.2/speccompare-api/get-parts.php?part=ram')
      .then((res) => res.json())
      .then((json) => {
  ...
  const getBrandFromName = (name) => {
    ...
    if (lower.includes('kingston')) return 'Kingston';
    if (lower.includes('corsair')) return 'Corsair';
    if (lower.includes('g.skill')) return 'G.SKILL';
    if (lower.includes('adata')) return 'ADATA';
    if (lower.includes('crucial')) return 'Crucial';
    if (lower.includes('patriot')) return 'Patriot';
    if (lower.includes('apacer')) return 'Apacer';
    if (lower.includes('lexar')) return 'Lexar';
    if (lower.includes('t-force') || lower.includes('team')) return 'T-Force';
    return 'Other';
  };

  const brands = ['Kingston', 'Corsair', 'G.SKILL', 'ADATA', 'Crucial', 'Patriot', 'Apacer', 'Lexar', 'T-Force'];
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>เลือก Memory</Text>
