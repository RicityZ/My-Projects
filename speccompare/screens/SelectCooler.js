...
export default function SelectCooler({ route, navigation }) {
  ...
  useEffect(() => {
    fetch('http://10.0.2.2/speccompare-api/get-parts.php?part=cooler')
      .then((res) => res.json())
      .then((json) => {
  ...
  const getBrandFromName = (name) => {
    ...
    if (lower.includes('be quiet')) return 'Be Quiet!';
    if (lower.includes('corsair')) return 'Corsair';
    if (lower.includes('noctua')) return 'Noctua';
    if (lower.includes('deepcool')) return 'DeepCool';
    return 'Other';
  };

  const brands = ['Cooler Master', 'Be Quiet!', 'Corsair', 'Noctua', 'DeepCool', 'Other'];
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>เลือก CPU Cooler</Text>
