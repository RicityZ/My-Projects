...
export default function SelectCPU({ route, navigation }) {
  ...
  useEffect(() => {
    fetch('http://10.0.2.2/speccompare-api/get-parts.php?part=cpu')
      .then((res) => res.json())
      .then((json) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>เลือก CPU</Text>
              ...
              <Image
                source={
                  brand === 'Intel'
                    ...
                    : require('../assets/amd.png')
                }
                style={styles.brandLogo}
              />
              <Text style={styles.brandText}>{brand.toUpperCase()}</Text>
            </View>
            {grouped[brand].map((cpu) => (
              <View key={cpu.id}>{renderItem({ item: cpu })}</View>
            ))}
          </View>
        )}
      />
